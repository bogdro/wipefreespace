...
static const char author_str[] = "Copyright (C) 2007-2026 Bogdan 'bogdro' Drozdowski, bogdro@users.sourceforge.net\n";
...
/**
 * Tells if verbose output was requested.
 * \return The verbosity level.
 */
int wfs_is_verbose (WFS_VOID)
{
	return opt_verbose;
}
...
/* ======================================================================== */
#ifndef WFS_ANSIC
int wfs_parse_cmdline WFS_PARAMS ((int argc, char* argv[]));
#endif
...
#endif
{
	...
	int i;
	int j;
	wfs_errcode_t ret = WFS_SUCCESS;	/* Value returned to main() ("last error") */
	...
	wfs_fsid_t wf_gen;
	wfs_errcode_t err;
	const char *wfs_progname;	/* The name of the program */
	...
	wfs_progname = wfs_get_program_name();

	if ( (argc <= 1) || (argv == NULL) )
	{
		if ( wfs_is_stdout_open() == 1 )
		{
			print_help ("");
		}
		return WFS_BAD_CMDLN;
...
# pragma GCC poison optind
	...
	if ( opt_bg == 1 )
	{
		...
		{
			opt_verbose = 0;
			wfs_set_stdout_open(0);
			wfs_set_stderr_open(0);
		}
#else
		puts ( msg_nobg );
#endif
